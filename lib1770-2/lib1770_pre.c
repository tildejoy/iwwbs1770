...
#define LIB1770_GET(buf, offs, i) ((buf)[lib1770_get(offs, i)])
#define LIB1770_GETX(buf, offs, i) LIB1770_GET(buf, offs, i)
#define LIB1770_GETY(buf, offs, i) LIB1770_GET(buf, (offs) - 6, i)
#define LIB1770_GETZ(buf, offs, i) LIB1770_GET(buf, (offs) - 3, i)
...
static const lib1770_biquad_t *
lib1770_f1_48000(void)
{
...
lib1770_pre_t *
lib1770_pre_new_lfe(double samplerate, int channels, int lfe)
#else
lib1770_pre_t *
lib1770_pre_new(double samplerate, int channels)
#endif
{
	lib1770_pre_t *pre;
	int i, ch;
	...
	pre->samplerate = samplerate;	/* set the sample frequency */
	pre->channels=channels;		/* set the number of channels */
	...
	for (i = 0, ch = 0; i < LIB1770_MIN(channels, LIB1770_MAX_CHANNELS); \
	    ++i) {
#if defined(LIB1770_LFE)
...
void 
lib1770_pre_close(lib1770_pre_t *pre)
{
	LIB1770_FREE(pre);
}

void 
lib1770_pre_add_block(lib1770_pre_t *pre, lib1770_block_t *block)
{
...
void 
lib1770_pre_add_sample(lib1770_pre_t *pre, lib1770_sample_t sample)
{
	...
	for (i=0, ch=0; i <LIB1770_MIN(channels, LIB1770_MAX_CHANNELS); ++i) {
		...
		if (1 < size) {
			double y = LIB1770_GETY(buf, offs, 0) = \
			    LIB1770_DEN(f1->b0 * x + f1->b1 * \
			    LIB1770_GETX(buf, offs, -1) + f1->b2 * \
			    LIB1770_GETX(buf, offs, -2) - f1->a1 * \
			    LIB1770_GETY(buf, offs, -1) - f1->a2 * \
			    LIB1770_GETY(buf, offs, -2));
			double z = LIB1770_GETZ(buf, offs, 0) = \
			    LIB1770_DEN(f2->b0 * y + f2->b1 * \
			    LIB1770_GETY(buf, offs, -1) + f2->b2 * \
			    LIB1770_GETY(buf, offs, -2) - f2->a1 * \
			    LIB1770_GETZ(buf, offs, -1) - f2->a2 * \
			    LIB1770_GETZ(buf, offs, -2));
			wssqs += (*g++) * z * z;
			++buf;
		}
...
void 
lib1770_pre_flush(lib1770_pre_t *pre)
{
	int channels = pre->channels;
#if defined(LIB1770_LFE)
	int lfe = pre->lfe;
	...
	if (1 < pre->ring.size) {
		for (i = 0, ch = 0; \
		    i < LIB1770_MIN(channels, LIB1770_MAX_CHANNELS); ++i) {
#if defined (LIB1770_LFE)
			/* filter out the lfe channel */
			if (lfe == i)
				continue;
