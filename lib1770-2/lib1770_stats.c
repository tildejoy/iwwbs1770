...
int 
lib1770_stats_create(lib1770_stats_t *stats)
{
	double step = 1.0 / LIB1770_HIST_GRAIN;
	lib1770_bin_t *wp, *mp;

#if 0
	stats = LIB1770_CALLOC(1, (sizeof *stats) + LIB1770_HIST_NBINS * \
		(sizeof stats->hist.bin[0]));
...
void 
lib1770_stats_destroy(lib1770_stats_t *stats)
{
	(void)stats;
}
#endif
...
void 
lib1770_stats_close(lib1770_stats_t *stats)
{
#if !defined(LIB1770_HIST_NBINS)
...
void 
lib1770_stats_merge(lib1770_stats_t *lhs, const lib1770_stats_t *rhs)
#else
void 
lib1770_stats_merge(lib1770_stats_t *lhs, lib1770_stats_t *rhs)
#endif 
{
	/*
	 * m1 = (sum_n c_n) / n
...
#endif
		lhs->hist.pass1.wmsq = q1 * lhs->hist.pass1.wmsq + q2 * \
		    rhs->hist.pass1.wmsq;
		bin1 = lhs->hist.bin;
		bin2 = rhs->hist.bin;
...
static int 
lib1770_bin_cmp(const void *key, const void *bin)
{
...
void 
lib1770_stats_add_sqs(lib1770_stats_t *stats, double wmsq)
{
	lib1770_bin_t *bin;
...
double 
lib1770_stats_get_max(lib1770_stats_t *stats)
{
	return LIB1770_LUFS(stats->max.wmsq);
}

double 
lib1770_stats_get_mean(lib1770_stats_t *stats, double gate)
{
...
double
lib1770_stats_get_range(lib1770_stats_t *stats, double gate, double lower,
    double upper)
{
	const lib1770_bin_t *rp, *mp;
	lib1770_count_t count = 0ull;
	...
	if (0ull < count) {
		...
		while (rp < mp) {
			if (gate < rp->x) {
				...
				if (prev_count < lower_count && \
				    lower_count <= count)
					min = rp->db;

				if (prev_count < upper_count && upper_count <= \
				    count) {
					max = rp->db;
					break;
				}
